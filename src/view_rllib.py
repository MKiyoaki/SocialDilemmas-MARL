...
from src.helper.configuration import output_dir, algo_name
...
def get_most_recent_folder(base_path):
    ...
    for folder in folders:
        try:
            timestamp_str = (folder.split("_")[-2] +
                             "_" + folder.split("_")[-1])
            ...
            print(f"Could not parse timestamp from folder name '{folder}': {e}")

    return os.path.join(base_path, most_recent_folder) if most_recent_folder else None
...
def main():
    ...
    parser.add_argument(
        "--experiment_state",
        type=str,
        default=f"{output_dir}/{algo_name}",
    ...
    agent_algorithm = algo_name

    register_env("meltingpot", utils.env_creator)

    experiment = ExperimentAnalysis(args.experiment_state)
    ...
    trainer = tune.registry.get_trainable_cls(agent_algorithm)(config=config)
    print(trainer)
