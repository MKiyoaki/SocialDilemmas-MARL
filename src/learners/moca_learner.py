...
class MOCALearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        # Instead of storing a fixed contract value, we now consider the contract as a function mapping.
        # We maintain contract parameters that parameterize the contract function.
        # Here, chosen_contract represents the current contract function parameter.
        self.contract_params_range = getattr(args, 'contract_params_range', (0.0, 1.0))
        # Sample initial contract parameters if not provided
        self.chosen_contract_params = getattr(args, 'chosen_contract_params', None)
        if self.chosen_contract_params is None:
            self.chosen_contract_params = np.random.uniform(low=self.contract_params_range[0],
                                                              high=self.contract_params_range[1])
        # 'contract_coef' determines the fallback influence of the contract.
        self.contract_coef = getattr(args, 'contract_coef', 0.0)

        # Initialize the contract instance.
        # Get transfer_function from args; if it's a string, convert it.
        ...
        # The contract instance is now a function mapping from features (or state) to a reward transfer vector.
        self.contract_instance = GeneralContract(num_agents=self.n_agents,
                                                 contract_type=contract_type,
                                                 params_range=self.contract_params_range,
                                                 transfer_function=transfer_fn)
        # Candidate contract sampling results storage.
        self.last_candidate_contracts = None
        self.last_candidate_scores = None
        # ------------------ End MOCA Contract Support ------------------

    def sample_candidate_contracts(self, batch: EpisodeBatch):
        """
        Sample candidate contract parameters from the defined range.
        Candidate contracts are now represented as parameter values for the contract function.
        """
        num_candidates = self.args.num_contract_candidates
        low_val = float(self.contract_instance.contract_space.low)
        high_val = float(self.contract_instance.contract_space.high)

        # Use the current chosen contract parameter as one candidate.
        candidate_list = []
        if low_val <= self.chosen_contract_params <= high_val:
            candidate_list.append(self.chosen_contract_params)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Use the contract instance to compute adjusted rewards.
        if self.contract_instance is not None:
            obs = batch["obs"]
            # Use the learner's chosen contract parameters as the current contract parameters.
            current_contract_params = self.chosen_contract_params
            # Compute adjusted rewards by calling the contract instance's compute_transfer.
            rewards = self.contract_instance.compute_transfer(obs, batch["actions"], rewards, current_contract_params)
        else:
            if self.contract_coef != 0.0:
                rewards = rewards * (1 + self.contract_coef * self.chosen_contract_params)
        # --------------------------------------------------
