...
def parallel_env(env_config, env_name, render_mode, max_cycles=MAX_CYCLES):
  return _ParallelEnv(env_config, env_name, render_mode, max_cycles)

def raw_env(env_config, env_name, max_cycles=MAX_CYCLES):
  return pettingzoo_utils.parallel_to_aec_wrapper(
    parallel_env(env_config, env_name, max_cycles))


def env(env_config, env_name, max_cycles=MAX_CYCLES):
  aec_env = raw_env(env_config, env_name, max_cycles)
...
class _MeltingPotPettingZooEnv(pettingzoo_utils.ParallelEnv):
  ...
  def __init__(self, env_config, env_name, max_cycles):
    self.env_config = config_dict.ConfigDict(env_config)
    self.env_name = env_name  # Store env_name
    self.max_cycles = max_cycles
    # Pass env_name instead of env_config to substrate.build if available
...
class _ParallelEnv(_MeltingPotPettingZooEnv, gym_utils.EzPickle):
  ...
  def __init__(self, env_config, env_name, render_mode='human', max_cycles=MAX_CYCLES):
    # Save the necessary parameters for pickling
    gym_utils.EzPickle.__init__(self, env_config, env_name, render_mode, max_cycles)

    self.render_mode = render_mode
    self.env_name = env_name
    self.max_cycles = max_cycles
    _MeltingPotPettingZooEnv.__init__(self, env_config, env_name, max_cycles)

  def __getstate__(self):
    """Override getstate to assist cloudpickle with serialization."""
    return (self.env_config, self.env_name, self.render_mode, self.max_cycles)

  def __setstate__(self, state):
    """Override setstate to assist cloudpickle with deserialization."""
    env_config, env_name, render_mode, max_cycles = state
    self.__init__(env_config, env_name, render_mode, max_cycles)